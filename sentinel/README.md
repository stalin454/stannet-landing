...
Limits: 10 MiB per file, 64 KiB rules, 32 rules/128 patterns, 5 seconds scanner timeout, 15 seconds worker work deadline, 30 seconds initialization deadline. Authenticode presence is detected but cryptographic trust/signature validation is not performed. VirusTotal reputation is disabled unless the Cloudflare secret VIRUSTOTAL_API_KEY is configured; only the SHA-256 is sent, never the file. Limits and truncation appear in reports. A low score is not proof that a file is safe. Demo rules are educational; no malware corpus precision claims, AI diagnosis, cloud sync, sandbox execution, reputation lookup, digital signature validation or complete PE/ELF parsing.
