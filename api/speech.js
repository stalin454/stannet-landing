export default async function handler(request, response) {
  ...
  const ssml = `<speak version="1.0" xml:lang="da-DK"><voice name="da-DK-ChristelNeural"><prosody rate="-8%">${safeText}</prosody></voice></speak>`;
  try {
    const azureResponse = await fetch(`https://${region}.tts.speech.microsoft.com/cognitiveservices/v1`, {
      ...
      headers: {
        ...
        'User-Agent': 'stannet-danish-coach',
      },
      body: ssml,
    });
